...
# murmity-backend.exe, standalone — quem for RODAR o .exe não
...
Copy-Item $nodePath "build\murmity-backend.exe" -Force

Write-Host "5/5 Injetando o código no executável (postject)..." -ForegroundColor Cyan
npx postject "build\murmity-backend.exe" NODE_SEA_BLOB "build\sea-prep.blob" `
  --sentinel-fuse NODE_SEA_FUSE_fce680ab2cc467b6e072b8b5df1996b2 `
  --overwrite

Write-Host "`nPronto: build\murmity-backend.exe" -ForegroundColor Green
Write-Host "Teste rodando: .\build\murmity-backend.exe" -ForegroundColor Green
